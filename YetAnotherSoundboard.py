...
class VoiceStudio(QWidget):
    ...
    def _apply_ui_to_engine(self) -> None:
        ...
        '''
        def get_colon_side(text: str, isRight: bool):
            colon_index = text.find(":")
            if isRight == True:
                right_text = text[colon_index:]
                return right_text
            if isRight == False:
                left_text = text[:colon_index]
                return int(left_text)

        input_text = self.input_combo.currentText()
        input_name = get_colon_side(input_text, True)
        input_intager = get_colon_side(input_text, False)
        p.input = input_intager
        print(f"from _apply_ui, in-{input_intager}|{input_name}")
        

        soundboard_text = self.soundboard_combo.currentText()
        soundboard_name = get_colon_side(soundboard_text, True)
        soundboard_intager = get_colon_side(soundboard_text, False)
        p.soundboard = soundboard_intager
        print(f"from _apply_ui, SB-{soundboard_intager}|{soundboard_name}")


        headphones_text = self.headphones_combo.currentText()
        if str(headphones_text) == "None":
            print('from _apply_ui, HP-0|: None')
            p.headphones = int(0)
        else:
            headphones_name = get_colon_side(headphones_text, True)
            headphone_intager = get_colon_side(headphones_text, False)
            p.headphones = headphone_intager
            print(f"from _apply_ui, HP-{headphone_intager}|{headphones_name}")
        '''
    ...
    def _export_current_preset(self) -> None:
        ...
        #QMessageBox.information(self, "Current preset JSON", txt)
